...
这是一个基于GraphQL Java实现 [规范](https://github.com/facebook/graphql) 
和 JavaScript [参考实现](https://github.com/graphql/graphql-js).
...
- [详情](#details)
- [鸣谢](#acknowledgment)
- [相关链接](#related-projects)
- [License](#license)
 

### 概述
 
这是一个Java实现GraphQL。这个库的目标是实际环境中使用的产品。

它负责解析和执行一个GraphQL查询。实际上它不照顾抓取任何数据: 数据来自实现回调或提供静态数据。  
  

### 规范

请注意,这个项目是与发布 [贡献者的行为准则](CODE_OF_CONDUCT.md).
导致这个项目(评论或打开公关/问题等)你同意遵循这一行为,所以请花时间去阅读它。
